...
RegexMatchList::RegexMatchList() : m_content(&m_emptyString)
{
}
...
{
    assert(this != &other);
    m_content = other.m_content == &other.m_emptyString
            ? &m_emptyString
            : other.m_content;
    ...
    m_regex(regex)
{
    ...
    int pos = 0;
    while (true) {
        re2::StringPiece piece(*m_content);
        ...
        pos = match.data() - m_content->data();
        if (match.length() > 0) {
            // SearchDFA finds the end of the match, but not the beginning.
            // Record the beginning of the searched chunk -- the actual
            // beginning will be computed lazily to speed up the initial
            // search.
            m_matchInitFlags.push_back(false);
            m_matchRanges.push_back(std::make_pair(pos, pos + match.length()));
            pos += match.length();
        } else {
            pos++;
            if (pos > piece.size())
...
const RegexMatchList::value_type &RegexMatchList::operator[](int i) const
{
    ...
    if (!m_matchInitFlags[i]) {
        ...
        re2::StringPiece piece(m_content->data() + start, end - start);
        if (m_reverseProg->SearchDFA(
            ...
            start = matchPiece.data() - m_content->data();
        } else {
